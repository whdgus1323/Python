# -*- coding: utf-8 -*-
"""
Backup 트리 (Collapsed + Depth Columns) - V17c
- 요구사항 반영:
  1) SAVE 단계도 '세부 진행률' 반영 (행 수/작업 수 기반으로 95%→100% 구간을 쪼갬)
  2) SAVE 단계에서도 실시간 로그 계속 출력
  3) [중지] 누르면 SAVE 도중이라도 '즉시' 중단 → 부분 파일 정리 후 종료
  4) 기본 경로 저장/불러오기, 실시간 로그/안전 종료 유지
"""
...
def pre_count_inventory(root: str, check_stop: Callable[[], bool], progress: Progress, log=print):
    ...
    # SAVE 총량은 저장 직전에 다시 set_totals(save_total=...)로 정교히 갱신
...
def estimate_save_total(df_paths: pd.DataFrame, df_summary: pd.DataFrame, df_members: pd.DataFrame) -> int:
    ...
    total = 10
    n_rows = len(df_paths)
    n_tree_rows = n_rows  # Tree_View와 동일 스케일
    n_sum = len(df_summary)
    n_mem = len(df_members)
    # 행 기반 추가: 500행당 1틱(너무 잦은 업데이트 방지)
...
def save_outputs(root_dir: str, df_paths: pd.DataFrame, level_cols: List[str],
    ...
    try:
        with pd.ExcelWriter(excel_path, engine="openpyxl") as w:
            ...
            # PatternMembers (많을 수 있음 → 청크틱)
            if not df_members.empty:
                # to_excel 자체는 일괄이므로, 진행률은 작성 전 청크틱으로 보정 어려움 → 로그만
    ...
    try:
        ...
        try:
            if os.path.exists(txt_collapsed_path):
                os.remove(txt_collapsed_path)
        except Exception:
            pass
