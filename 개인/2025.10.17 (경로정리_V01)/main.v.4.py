# -*- coding: utf-8 -*-
"""
Backup 트리 (Collapsed + Depth Columns) - V17b (진행률 보정/실시간 로그/기본경로 저장/안전종료)
- 실행 시 기본 경로 자동 세팅(저장값 없으면 사용자 홈)
- [경로 변경…] + "이 경로를 기본값으로 저장" 체크 지원 (APPDATA\BackupTree\config.json)
- 마우스 클릭 없이 백그라운드 스레드로 자동 진행, 실시간 로그 출력
- 진행률: COUNT→SCAN→BUILD 합쳐도 95%까지만, SAVE 완료 시 100%
- SAVE 단계 진입/완료 로그 확실히 출력, 예외 발생 시 팝업/로그 즉시 표시
- 창 닫기 시 안전 중단 후 프로세스 완전 종료
"""
...
class Progress:
    ...
    def set_totals(self, count_total=None, scan_total=None, build_total=None, save_total=None):
        ...
        if save_total  is not None: self.save_total  = max(1, save_total)
        self.emit("총 작업량 초기화")
    ...
    def tick_save (self, msg=""): self.save_done  += 1; self.emit(msg)
    ...
    def emit(self, msg=""):
        ...
        # SAVE 단계 전에는 95%를 넘지 않게 제한해 '완료처럼 보이는' 현상 방지
        if self.save_done == 0:
            pct = min(pct, 95.0)
...
def save_outputs(root_dir: str, df_paths: pd.DataFrame, level_cols: List[str],
    ...
    with pd.ExcelWriter(excel_path, engine="openpyxl") as w:
        ...
        progress.tick_save("[SAVE] Tree_Collapsed 시트")
        ...
        if "Tree_Collapsed" in wb.sheetnames: style_ws(wb["Tree_Collapsed"], wide_first=True)
...
def launch_gui():
    ...
    class App:
        ...
        def on_stop(self):
            ...
            self.btn_stop.config(state="disabled")
            self.log("[STOP] 중지 요청… 가능한 지점에서 안전 중단")
