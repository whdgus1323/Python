...
g['OUTPUT_DIR'] = g['BASE_DIR'] / 'outputs_v13_weight_tuning_verify_fast3'
...
orig_evaluate_candidate = g['evaluate_candidate']

def safe_evaluate_candidate(base_df, coeff):
    try:
        return orig_evaluate_candidate(base_df, coeff)
    except ValueError as e:
        if '0 sample' not in str(e):
            raise
        teacher, train_pair = g['build_teacher_and_pair'](base_df, coeff)
        stage1 = g['summarize_teacher_structure'](train_pair, teacher, coeff)
        return {
            **stage1,
            'best_val_loss': float('inf'),
            'row_r2_low': float('-inf'),
            'row_r2_high': float('-inf'),
            'row_r2_uniform': float('-inf'),
            'row_mae_low': float('inf'),
            'row_mae_high': float('inf'),
            'state_r2_low': float('-inf'),
            'state_r2_high': float('-inf'),
            'state_r2_uniform': float('-inf'),
            'state_mae_low': float('inf'),
            'state_mae_high': float('inf'),
            'train_rows': 0,
            'valid_rows': 0,
            'test_rows': 0,
            'eval_states': 0,
...
g['evaluate_candidate'] = safe_evaluate_candidate

exec(compile(code_cells[5], 'cell5.py', 'exec'), g)
base_df = g['base_df']
parts = []
for split, n in [('train', 100000), ('valid', 30000), ('test', 30000)]:
...
print('FAST3_VERIFY_DONE', flush=True)
